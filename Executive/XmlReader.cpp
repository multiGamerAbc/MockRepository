...
std::vector<TestElement> XmlReader::parseXmlIntoVectorOfTestElements() {
	...
	std::vector<std::string> testedCodeVec;

	std::vector<TestElement> testVec;
	while (next()) {
		while (next()) {
			if (tag() == "TestElement") {
				testVec.push_back(TestElement(testDriver, testedCodeVec));
				testedCodeVec.clear();
			}
...
std::vector<TestElement> XmlReader::parseXmlIntoVectorOfTestElements() {
	...
	while (next()) {
		while (next()) {
			if (tag() == "TestElement") {
				TestElement tE(testDriver, testedCodeVec);
				testVec.push_back(tE);
			}
			else if (tag() == "TestDriverElem") {
				testDriver = body();
			}
...
int main() {
	std::string str = "<TestElement>"
		...
		;
	XmlReader rdr(str);
	std::vector<TestElement> vectorTestElements = rdr.parseXmlIntoVectorOfTestElements();
	cout << endl;
	for (TestElement tE : vectorTestElements)
		tE.print();
	cin.get();
}
