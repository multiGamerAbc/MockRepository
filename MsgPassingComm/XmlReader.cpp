...
// version 2.48_ (modifed for Local Test Harness project)            //
// Language:    C++								                     //
...
///////////////////////////////////////////////////////////////////////
// Modifications:                                                    // 
// added parseXmlIntoVectorOfTestElements                            //
// added cin.get() to test stub                                      //
///////////////////////////////////////////////////////////////////////
...
//----< parse XML string into vector of TestElement >---------------------------

std::vector<TestElement> XmlReader::parseXmlIntoVectorOfTestElements() {

	std::string testDriverDllSpec;
	std::vector<std::string> testedCodeDllSpecVec;
	std::vector<TestElement> TestElementVec;

	while (next()) {
		while (next()) {
			if (tag() == "TestElement") {
				TestElementVec.push_back
				    (TestElement(testDriverDllSpec, testedCodeDllSpecVec));
				testedCodeDllSpecVec.clear();
			}
			else if (tag() == "TestDriverElem") {
				testDriverDllSpec = body();
			}
			else {
				testedCodeDllSpecVec.push_back(body());
			}
		}
		TestElementVec.push_back
		    (TestElement(testDriverDllSpec, testedCodeDllSpecVec));
	}
	return TestElementVec;
}
