...
std::vector<TestElement> parseXmlIntoVectorOfTestElements(std::string xmlStr) {
	...
	while (rdr.next()) {
		while (rdr.next()) {
			if (rdr.tag() == "TestElement") {
				TestElement tE(testDriver, testedCodeVec);
				testVec.push_back(tE);
			}
			else if (rdr.tag() == "TestDriverElem") {
				testDriver = rdr.body();
			}
			else {
				testedCodeVec.push_back(rdr.body());
			}
		}
		TestElement tE(testDriver, testedCodeVec);
		testVec.push_back(tE);
...
int main() {
	std::string str = "<TestElement>"
		...
		"</TestElement>"
		;
	std::vector<TestElement> vectorTestElements = parseXmlIntoVectorOfTestElements(str);
	cout << endl;
