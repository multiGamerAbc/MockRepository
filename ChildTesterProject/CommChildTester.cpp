...
// Cite:  https://stackoverflow.com/questions/10150468/how-to-redirect-cin-and-cout-to-files
// Cite: http://forums.codeguru.com/showthread.php?225533-Redirecting-COUT-from-standard-output-to-a-file-in-C

#include <algorithm>
#include "CommChildTester.h"
#include <conio.h>
#include <exception>
...
const std::string sendFilePath = "not used";
const size_t BlockSize = 1024;
Socket::byte rwBuffer[BlockSize];

//----< variables (set 2) >-----------------------------------------------------

std::string thisChildTesterAssignedID_String = "-1";

//----< executes test requests >------------------------------------------------

int main(int argc, char * argv[])
{
	int thisChildTesterAssignedID = -1;
	int thisTestRequestID = -1;
	int ctpPortNumForCtpServerCommunication = 3000;
	int ctpPortNumForCtpUserCommunication = 3500;
	int guiPortNumForCtpGuiCommunication = 9002;
	int serverPortNumForCtpServerCommunication = 7000;
	int aUserPortNumForCtpUserCommunication = 5000;
	int contrivedExtractorDelayTime = 20;
	// modify port numbers using this child tester process's id
	if (argc > 1) {
		thisChildTesterAssignedID_String = argv[0];
		thisChildTesterAssignedID = stoi(argv[0]);
		ctpPortNumForCtpServerCommunication += thisChildTesterAssignedID;
		ctpPortNumForCtpUserCommunication += thisChildTesterAssignedID;
		serverPortNumForCtpServerCommunication += thisChildTesterAssignedID;
		aUserPortNumForCtpUserCommunication += thisChildTesterAssignedID;
	}
	// modify saveFilePath for this child tester process
	ostringstream oss;
	oss << saveFilePath << "_" << thisChildTesterAssignedID_String;
	saveFilePath.clear();
	saveFilePath.append(oss.str());
	// create dedicated Comms
	SocketSystem ss;
	EndPoint guiEPforCtpGuiCommunication("localhost", guiPortNumForCtpGuiCommunication);
	EndPoint ctpEPforCtpServerCommunication("localhost", ctpPortNumForCtpServerCommunication);
	EndPoint ctpEPforCtpUserCommunication("localhost", ctpPortNumForCtpUserCommunication);
	EndPoint serverEPforCtpServerCommunication("localhost", serverPortNumForCtpServerCommunication);
	EndPoint aUserEPforCtpUserCommunication("localhost", aUserPortNumForCtpUserCommunication);
	Comm ctpServerComm(ctpEPforCtpServerCommunication, "serverUserComm");
	Comm ctpUserComm(ctpEPforCtpUserCommunication, "serverCtpComm");
	ctpServerComm.start();
	ctpUserComm.start();
	// Request and receive files from user, run tests
	while (true) {
		// send ready message to server
		Message readyMessage(serverEPforCtpServerCommunication, ctpEPforCtpServerCommunication);
		readyMessage.command("READY_MESSAGE");
		ostringstream oss;
		oss << ctpPortNumForCtpServerCommunication;
		std::string name = oss.str();
		readyMessage.name(name);
		ctpServerComm.postMessage(readyMessage);
		// get test request message from server
		Message msg = ctpServerComm.getMessage();
		std::string testRequest = msg.attributes().at("BODY");
		std::string testLogFileNameString = "TestLog_" + msg.attributes().at("testRequestID");
		ofstream redirect_file("../MockRepo/TestLogs/" + testLogFileNameString);
		// save output buffer of cout
		streambuf * strm_buffer = cout.rdbuf();
		// redirect output into file
		cout.rdbuf(redirect_file.rdbuf());
		if (msg.command() == "TEST_REQUEST") {// ideally, would check for stop message
			std::cout << "---> Now processing Test Request from: " << msg.name() << std::endl;
			// parse test request string into vector of TestElements
			XmlReader rdr(testRequest);
			std::vector<TestElement> teVec = rdr.parseXmlIntoVectorOfTestElements();
			std::vector<TestElement> teVecWithFileExtensions;
			std::vector<TestElement> teVecWithPrefixedPathAndFileExtensions;
			// affix file extensions
			for (TestElement tE : teVec)
				teVecWithFileExtensions.push_back(tE.addFileExtensions(".dll"));
			// prefix save paths
			for (TestElement tE : teVecWithFileExtensions)
				teVecWithPrefixedPathAndFileExtensions.push_back(tE.prefixPath("../SaveFiles_" + thisChildTesterAssignedID_String + "/"));  // OK
			// re-create test request string with prefixed paths and affixed file extensions
			std::string path_ModifiedTestRequest = "";
			for (TestElement tE : teVecWithPrefixedPathAndFileExtensions)
				path_ModifiedTestRequest += tE.toString();
			// request all needed files from user, keep request count
			int fileRequestCount = 0;
			for (TestElement tE : teVecWithFileExtensions) {
				fileRequestCount++;
				Message msg(aUserEPforCtpUserCommunication, ctpEPforCtpUserCommunication);
				msg.command("FILE_REQUEST");
				msg.attribute("file_requested", tE.testDriverElement);
				ostringstream oss;
				oss << ctpPortNumForCtpServerCommunication;
				std::string name = oss.str();
				msg.name(name);
				ctpUserComm.postMessage(msg);
				for (std::string testedCodeElement : tE.testedCodeElements) {
					fileRequestCount++;
					Message msg(aUserEPforCtpUserCommunication, ctpEPforCtpUserCommunication);
					msg.command("FILE_REQUEST");
					msg.attribute("file_requested", testedCodeElement);
					ctpUserComm.postMessage(msg);
				}
			}
			// receive all needed files from user, keep receive count
			int fileReceivedCount = 0;
			while (true) {
				Message msg = ctpUserComm.getMessage();
				if (msg.command() == "FILE_MESSAGE") {
					//std::cout << "Received file message: " << msg.name() << endl;
				}
				if (msg.containsKey("file")) {
					if (msg.contentLength() == 0) {
						Message fileCompleteMsg(aUserEPforCtpUserCommunication, ctpEPforCtpUserCommunication);
						fileCompleteMsg.command("FILE_COMPLETE");
						fileCompleteMsg.name("my name");
						ctpUserComm.postMessage(fileCompleteMsg);
						fileReceivedCount++;
						if (fileReceivedCount == fileRequestCount) {
							break;
						}
					}
				}
				std::this_thread::sleep_for(std::chrono::milliseconds(contrivedExtractorDelayTime));
			}
			// send path_ModifiedTestRequest string to TestHarness object
			TestHarness harness;
			harness.processAllElements(path_ModifiedTestRequest);
			MyLogger myLogger;
			// Create a time string that can be extracted from CsMessage in GUI
			const std::string tmp = myLogger.time().str();
			const std::string timeString = tmp.substr(0, 17); 

			std::string testLogFileString = "TestLog_" + msg.attributes().at("testRequestID");
			Message testStatusMsg;
			testStatusMsg.to(guiEPforCtpGuiCommunication);
			testStatusMsg.command("Test Status Message");
			testStatusMsg.attribute("TimeDateStamp", timeString);
			testStatusMsg.attribute("TestLogFileString", testLogFileString);
			testStatusMsg.attribute("testRequestID", msg.attributes().at("testRequestID"));
			testStatusMsg.attribute("status", "complete");
			ctpUserComm.postMessage(testStatusMsg); // We can share Comms
		}
		std::this_thread::sleep_for(std::chrono::milliseconds(contrivedExtractorDelayTime));
		// restore old buffer
